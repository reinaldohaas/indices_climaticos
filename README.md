# 🌍 Índices Climáticos (NOAA PSL, NCEI ERSST v5, CPC MJO & SILSO SSN)
...
## 📌 Fontes de Dados
Os dados são coletados e continuamente atualizados a partir das seguintes fontes oficiais internacionais:
...
3. **NOAA Climate Prediction Center (CPC - Teleconnects & MJO):**
   - Teleconexões & ONI/SOI: [https://www.cpc.ncep.noaa.gov/data/indices/](https://www.cpc.ncep.noaa.gov/data/indices/)
   - MJO (Madden-Julian Oscillation): [https://www.cpc.ncep.noaa.gov/products/precip/CWlink/daily_mjo_index/proj_norm_order.ascii](https://www.cpc.ncep.noaa.gov/products/precip/CWlink/daily_mjo_index/proj_norm_order.ascii)
4. **SILSO / Royal Observatory of Belgium (SSN - Sunspot Number):** [https://www.sidc.be/SILSO/datafiles](https://www.sidc.be/SILSO/datafiles)
...
| **Oscilação de Madden-Julian (MJO)** | `mjo_pentad.asc` (Pentadas brutas), `mjo_20e.asc`, `mjo_70e.asc`, `mjo_80e.asc`, `mjo_100e.asc`, `mjo_120e.asc`, `mjo_140e.asc`, `mjo_160e.asc`, `mjo_120w.asc`, `mjo_40w.asc`, `mjo_10w.asc` | **1978 – 2026+** |
| **Atividade Solar (SSN)** | `ssn.asc` (Monthly Sunspot Number matricial), `ssn_monthly_raw.asc` (SILSO V2.0 bruto) | **1749 – 2026+** |
...
```
indices_climaticos/
├── download_climate_indices.py        # Script para baixar a lista base da NOAA PSL
├── update_climate_indices_ersst_v5.py # Script para atualizar os índices com ERSST v5, CPC (MJO) e SILSO (SSN)
...
└── indices_asc/                        # Pasta contendo os arquivos .asc e o dataset unificado
    ├── catalog.csv                     # Inventário de todos os arquivos .asc
    ├── catalog.json                    # Inventário em formato JSON
    ├── all_climate_indices_monthly.csv # Dataset unificado em CSV (117.000+ observações)
    ├── mjo_pentad.asc                  # MJO pentadal bruto da NOAA CPC
    ├── mjo_120e.asc                    # MJO mensal na longitude 120ºE
    ├── ssn.asc                         # Número de manchas solares SILSO (1749-2026)
    ├── pna.asc                         # Arquivos individuais dos demais índices
    └── ...
```
...
Para atualizar os arquivos `.asc` e o dataset CSV com as informações mais recentes:

```bash
# 1. Baixar lista completa da NOAA PSL
python download_climate_indices.py

# 2. Atualizar índices com NOAA NCEI ERSST v5, NOAA CPC (MJO) & SILSO (SSN) (2026+)
...
Dados de domínio público disponibilizados pela **NOAA** (National Oceanic and Atmospheric Administration) e **SILSO** (Sunspot Index and Long-term Solar Observations - Royal Observatory of Belgium).
